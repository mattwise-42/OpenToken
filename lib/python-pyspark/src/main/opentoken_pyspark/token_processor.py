...
class OpenTokenProcessor:
    ...
    def process_dataframe(self, df: DataFrame) -> DataFrame:
        ...
        def generate_tokens_udf(
            ...
            # Load attribute classes
            attribute_map = {
                RecordIdAttribute: RecordIdAttribute(),
                FirstNameAttribute: FirstNameAttribute(),
                LastNameAttribute: LastNameAttribute(),
                BirthDateAttribute: BirthDateAttribute(),
                SexAttribute: SexAttribute(),
                PostalCodeAttribute: PostalCodeAttribute(),
                SocialSecurityNumberAttribute: SocialSecurityNumberAttribute(),
            }
            ...
            for idx in range(len(record_id_series)):
                ...
                person_attrs = {
                    RecordIdAttribute: str(record_id_series.iloc[idx]) if pd.notna(record_id_series.iloc[idx]) else None,
                    FirstNameAttribute: str(first_name_series.iloc[idx]) if pd.notna(first_name_series.iloc[idx]) else None,
                    LastNameAttribute: str(last_name_series.iloc[idx]) if pd.notna(last_name_series.iloc[idx]) else None,
                    BirthDateAttribute: str(birth_date_series.iloc[idx]) if pd.notna(birth_date_series.iloc[idx]) else None,
                    SexAttribute: str(sex_series.iloc[idx]) if pd.notna(sex_series.iloc[idx]) else None,
                    PostalCodeAttribute: str(postal_code_series.iloc[idx]) if pd.notna(postal_code_series.iloc[idx]) else None,
                    SocialSecurityNumberAttribute: str(ssn_series.iloc[idx]) if pd.notna(ssn_series.iloc[idx]) else None,
                }
